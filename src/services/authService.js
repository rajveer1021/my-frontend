...
export const authService = {
  ...
  async signup(userData) {
    try {
      const response = await apiService.post("/auth/signup", {
        ...
        accountType:
          userData.userType?.toUpperCase() || userData.accountType || "VENDOR",
      });
  ...
  async googleAuth(googleCredential, accountType = "VENDOR") {
    try {
      ...
      if (response.success && response.needsAccountTypeSelection) {
        ...
        try {
          const setAccountTypeResponse = await apiService.post(
            "/auth/google/set-account-type",
            {
              ...
              userInfo: {
                firstName:
                  ...
                  response.userInfo.name?.split(" ")[0] ||
                  "User",
                lastName:
                  response.userInfo.lastName ||
                  response.userInfo.name?.split(" ").slice(1).join(" ") ||
                  "",
          ...
          if (
            setAccountTypeResponse.success &&
            setAccountTypeResponse.data &&
            setAccountTypeResponse.data.token
          ) {
            const authToken = setAccountTypeResponse.data.token;
            localStorage.setItem("authToken", authToken);

            const user = this.transformUserData(
              setAccountTypeResponse.data.user
            );
            ...
            return {
              ...
              token: authToken,
              message:
                setAccountTypeResponse.message ||
                "Account created successfully with VENDOR access",
            };
          }
        } catch (setAccountError) {
          console.error(
            "❌ Failed to set account type automatically:",
            setAccountError
          );
      ...
      if (error.status) {
        ...
        switch (status) {
          ...
          case 409:
            errorMessage =
              serverMessage ||
              "An account with this email already exists with a different account type";
            break;
          case 422:
            errorMessage =
              serverMessage || "Invalid account type or missing data";
            break;
          case 500:
            errorMessage =
          ...
          default:
            errorMessage =
              serverMessage || `Authentication failed (Error ${status})`;
        }
      } else if (
  ...
  transformUserData(apiUserData) {
    ...
    return {
      ...
      isAdmin: apiUserData.accountType === "ADMIN",
      permissions: apiUserData.accountType === "ADMIN" ? ["all"] : [],
    };
  },
  ...
  isValidUserData(userData) {
    ...
    // Admin users have different validation requirements
    ...
    // Regular validation for vendor/buyer users
    return !!(
      userData.id &&
      userData.email &&
      userData.email !== "undefined" &&
  ...
  // Add method to check admin permissions
  hasAdminAccess(user) {
    return user && user.accountType === "ADMIN";
  },

  // Add method to get redirect URL after login
  getPostLoginRedirect(user) {
    ...
    switch (user.accountType) {
      ...
      case "VENDOR":
        return "/";
      case "BUYER":
        return "/marketplace"; // If you have a buyer dashboard
      default:
        return "/";
    }
  },
  ...
  cleanupInvalidToken() {
    ...
    if (
      token &&
      (this.isTokenExpired(token) || !this.isValidTokenFormat(token))
    ) {
      this.logout();
      return true;
    }
