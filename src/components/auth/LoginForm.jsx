// src/components/auth/LoginForm.jsx - Fixed version with proper Google OAuth configuration
...
const LoginForm = ({ onSwitchToSignup, onForgotPassword }) => {
  ...
  const { login, loading, error, clearError, googleLogin, setGoogleUserAccountType } = useAuth();
  const { addToast } = useToast();

  // Remove Google One Tap for now to avoid FedCM issues
  ...
  // UPDATED: Google Success Handler with better error handling
  const handleGoogleSuccess = async (credentialResponse) => {
    ...
    try {
      ...
      if (result.needsAccountTypeSelection) {
        console.log('🎯 Account type selection needed');
  ...
  // Handle account type confirmation
  const handleAccountTypeConfirm = async (accountType) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        addToast(result.message || "Welcome to Multi-Vendor!", "success");
      }
    } catch (error) {
